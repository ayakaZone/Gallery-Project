...
import static com.yupi.yupicture.domain.picture.valueobject.PictureReviewStatusEnum.PASS;
...
public class PictureApplicationServiceImpl implements PictureApplicationService {
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User user) {
        ...
        Picture execute = transactionTemplate.execute(status -> {
            ...
            if (ObjUtil.isNotEmpty(finalSpaceId)) {
                // 更新空间容量和项目数量-自定义SQL
                LambdaUpdateWrapper<Space> spaceLambdaUpdateWrapper = new LambdaUpdateWrapper<Space>()
                        .eq(Space::getId, finalSpaceId).setSql("totalSize = totalSize + " + picture.getPicSize())
                        .setSql("totalCount = totalCount + 1");
                boolean updateResult = spaceApplicationService.spaceLambdaUpdate(spaceLambdaUpdateWrapper);
    ...
    public void deletePicture(DeleteRequest deleteRequest, User loginUser) {
        ...
        transactionTemplate.execute(status -> {
            ...
            if (ObjUtil.isNotEmpty(spaceId)) {
                // 更新空间容量和项目数量-自定义SQL
                LambdaUpdateWrapper<Space> spaceLambdaUpdateWrapper = new LambdaUpdateWrapper<Space>().eq(Space::getId, spaceId)
    ...
    public PictureVO getPictureVOById(long id, HttpServletRequest request) {
        ...
        if (ObjUtil.isNotEmpty(picture.getSpaceId())) { // 私有空间图片
            ...
            space = spaceApplicationService.getSpaceById(picture.getSpaceId());
