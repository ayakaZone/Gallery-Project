...
public class UserDomainServiceImpl implements UserDomainService {
    ...
    public boolean isVipUser(Long userId) {
        ...
        // 检查是否有会员编号且未过期
        if (user.getVipNumber() == null || user.getVipExpireTime() == null) {
            return false;
        }
        // 检查是否已过期
    ...
    public synchronized boolean validateAndUseCode(String vipCode) {
        try {
            // 1. 直接读取resources下的文件（最简单）
            String json = ResourceUtil.readUtf8Str("riz/vipCode.json");
     ...
     * 保存到外部文件（config目录）
     */
    private void saveToExternalFile(Map<String, Object> data) {
        try {
            File externalFile = new File("config/vipCode.json");
            ...
            FileUtil.writeUtf8String(newJson, externalFile);
        } catch (Exception e) {
            // 忽略保存异常
        }
    }
