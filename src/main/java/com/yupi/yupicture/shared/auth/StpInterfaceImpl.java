...
public class StpInterfaceImpl implements StpInterface {
    ...
    public List<String> getPermissionList(Object loginId, String loginType) {
        ...
        if (spaceUserId != null) {
            ...
            // 查询当前团队空间是否存在该用户
            LambdaQueryWrapper<SpaceUser> queryWrapper = new LambdaQueryWrapper<SpaceUser>()
                    .eq(SpaceUser::getSpaceId, spaceUser.getSpaceId())
                    .eq(SpaceUser::getUserId, userId);
            SpaceUser loginSpaceUser = spaceUserApplicationService.lambdaQuerySpaceUser(queryWrapper);
            // 团队空间不存在该用户—无权限
        ...
        if (space.getSpaceType().equals(SpaceTypeEnum.PRIVATE.getValue())) {
            ...
            LambdaQueryWrapper<SpaceUser> queryWrapper = new LambdaQueryWrapper<SpaceUser>()
                    ...
                    .eq(SpaceUser::getUserId, userId);
            spaceUser = spaceUserApplicationService.lambdaQuerySpaceUser(queryWrapper);
            // 用户不是该团队成员——无权限
            if (spaceUser == null) {
                return new ArrayList<>();
            }
